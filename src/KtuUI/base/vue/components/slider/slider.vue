<template>
    <div :class="classes">
        ...
        >
            ...
            <!-- 显示零刻度 -->
            <template v-if="showZeroStep">
                <div
                    :class="[prefixCls + '-zero']"
                    :style="zeroStyle"
                ></div>
            </template>
...
export default {
    ...
    watch: {
        value(val) {
            ...
            if (
                !this.dragging &&
                (val[0] !== this.currentValue[0] ||
                    val[1] !== this.currentValue[1])
            ) {
    ...
    computed: {
        ...
        exportValue() {
            ...
            return this.currentValue.map(nr =>
                Number(nr.toFixed(decimalCases))
            );
        },
        minPosition() {
        ...
        barStyle() {
            const style = {
                width:
                    ((this.currentValue[0] - this.min) / this.valueRange) *
                        100 +
                    "%"
            };
            ...
            if (this.range) {
                ...
                style.width =
                    ((this.currentValue[1] - this.currentValue[0]) /
                        ...
                        100 +
                    "%";
            } else if (this.showZeroStep) {
                if (this.currentValue[0] > 0) {
                    style.left = (-this.min / this.valueRange) * 100 + "%";
                } else {
                    style.right = (this.max / this.valueRange) * 100 + "%";
                }

                style.width =
                    (Math.abs(this.currentValue[0]) / this.valueRange) * 100 +
                    "%";
            }
        ...
        zeroStyle() {
            const style = {};
            const width = this.valueRange;

            style.left = Math.round((Math.abs(this.min) / width) * 100) + "%";
    ...
    methods: {
        ...
        onPointerDrag(event) {
            ...
            const diff =
                ((this.currentX - this.startX) / this.sliderWidth) *
                this.valueRange;
        ...
        sliderClick(event) {
            ...
            const sliderOffsetLeft = this.$refs.slider.getBoundingClientRect()
                .left;
            let newPos =
                ((currentX - sliderOffsetLeft) / this.sliderWidth) *
                    this.valueRange +
                this.min;
        ...
        handleSetSliderWidth() {
            this.sliderWidth = parseInt(
                getStyle(this.$refs.slider, "width"),
                10
            );
        }
    },
