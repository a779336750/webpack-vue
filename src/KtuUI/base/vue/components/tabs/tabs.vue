...
export default {
    ...
    props: {
        ...
        type: {
            validator(value) {
                return oneOf(value, ["line", "card", "custom"]);
            },
            default: "line"
        },
        size: {
    ...
    computed: {
        classes() {
            return [
                `${prefixCls}`,
                {
                    ...
                    [`${prefixCls}-mini`]:
                        this.size === "small" && this.type === "line",
                    [`${prefixCls}-no-animation`]: !this.animated,
                    [`${prefixCls}-button`]: this.isButton,
                }
            ];
        },
        contentClasses() {
        ...
        barStyle() {
            ...
            let style = {
                visibility: "hidden",
                width: `${this.barWidth - marginWidth * 2}px`
            };
            if (this.type === "line") style.visibility = "visible";


            if (this.animated) {
                style.transform = `translate3d(${this.barOffset + marginWidth}px, 0px, 0px)`;
            } else {
    ...
    methods: {
        ...
        updateBar() {
            this.$nextTick(() => {
                ...
                if (index > 0) {
                    let offset = 0;
                    const gutter = this.size === "small" ? 0 : 0;
        ...
        updateStatus() {
            ...
            tabs.forEach(
                tab =>
                    (tab.show =
                        tab.currentName === this.activeKey || this.animated)
            );
        },
        tabCls(item) {
            return [
            ...
            if (tab.currentName === this.activeKey) {
                ...
                if (newTabs.length) {
                    ...
                    if (rightNoDisabledTabs.length) {
                        ...
                        activeKey =
                            leftNoDisabledTabs[leftNoDisabledTabs.length - 1]
                                .currentName;
                    } else {
        ...
        updateVisibility(index) {
            [
                ...this.$refs.panes.querySelectorAll(`.${prefixCls}-tabpane`)
            ].forEach((el, i) => {
                if (index === i) {
                    [...el.children]
                        .filter(child =>
