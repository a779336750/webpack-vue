...
export default {
    ...
    watch: {
        ...
        currentView(currentView) {
            ...
            const isSameYear =
                this.leftPanelDate.getFullYear() ===
    ...
    methods: {
        ...
        setPanelDates(leftPanelDate) {
            ...
            const splitRightPanelDate = this.dates[1]
                ? this.dates[1].getTime()
                : this.dates[1];
            this.rightPanelDate = this.splitPanels
                ? new Date(
                      Math.max(splitRightPanelDate, rightPanelDate.getTime())
                  )
                : rightPanelDate;
        },
        panelLabelConfig(direction) {
            ...
            return {
                separator: separator,
                labels: labels.map(
                    obj => ((obj.handler = handler(obj.type)), obj)
                )
            };
        },
        prevYear(panel) {
        ...
        changePanelDate(panel, type, increment, updateOtherPanel = true) {
            ...
            if (this.splitPanels) {
                ...
                if (
                    panel === "left" &&
                    this.leftPanelDate >= this.rightPanelDate
                ) {
                    this.changePanelDate(otherPanel, type, 1);
                }
                if (
                    panel === "right" &&
                    this.rightPanelDate <= this.leftPanelDate
                ) {
                    this.changePanelDate(otherPanel, type, -1);
                }
            } else {
