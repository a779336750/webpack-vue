...
const pulseElement = el => {
    const pulseClass = "date-picker-btn-pulse";
    ...
    methods: {
        ...
        handleKeydown(e) {
            ...
            if (keyCode === 9) {
                if (this.visible) {
                    ...
                    if (this.isConfirm) {
                        ...
                        const tabbable = this.$refs.drop.$el.querySelectorAll(
                            selector
                        );
            ...
            if (keyCode === 13) {
                ...
                if (this.type.match(/range/)) {
                    this.$refs.pickerPanel.handleRangePick(
                        this.focusedDate,
                        "date"
                    );
                } else {
                    ...
                    const compareDate = d => {
                        const sliceIndex =
                            ["year", "month", "date"].indexOf(this.type) + 1;
                    ...
                    const dateIsValid = panels.find(({ cells }) => {
                        return cells.find(
                            ({ date, disabled }) =>
                                compareDate(date) ===
                                    compareDate(this.focusedDate) && !disabled
                        );
                    });
                    if (dateIsValid)
        ...
        navigateTimePanel(direction) {
            ...
            const column = (currentColumn => {
                const incremented =
                    currentColumn +
                    (horizontal ? (direction === "left" ? -1 : 1) : 0);
            ...
            if (vertical) {
                ...
                const nextIndex =
                    pickerPossibleValues.findIndex(
                        ({ text }) =>
                            this.focusedTime.time[pickerIndex][col] === text
                    ) + increment;
                const nextValue =
                    pickerPossibleValues[
                        nextIndex % pickerPossibleValues.length
                    ].text;
        ...
        navigateDatePanel(direction, shift) {
            ...
            if (shift) {
                if (this.type === "year") {
                    ...
                    this.focusedDate = new Date(
                        this.focusedDate.getFullYear() +
                            mapPossibleValues(direction, 0, 1),
                        this.focusedDate.getMonth() +
                            mapPossibleValues(direction, 1, 0),
                        this.focusedDate.getDate()
                    );
                }
            ...
            if (this.type.match(/^date/)) {
                ...
                if (nextDay < 1) {
                    if (direction.match(/left|right/)) {
                        ...
                        focusedDate.setDate(
                            startDay +
                                Math.floor((lastOfMonth - startDay) / 7) * 7
                        );
                    }
            ...
            if (this.type.match(/^year/)) {
                focusedDate.setFullYear(
                    focusedDate.getFullYear() +
                        mapPossibleValues(direction, 1, 3)
                );
            }
            ...
            if (val && type === "time" && !(val instanceof Date)) {
                ...
                if (!val) {
                    ...
                    if (typeof val === "string") {
                        ...
                        val = parser(val, format, this.separator).map(
                            v => v || ""
                        );
                    } else {
                        const [start, end] = val;
        ...
        onPick(dates, visible = false, type) {
            if (this.multiple) {
                ...
                const timeStamps = allDates
                    ...
                    .filter(
                        (ts, i, arr) =>
                            arr.indexOf(ts) === i && i !== indexOfPickedDate
